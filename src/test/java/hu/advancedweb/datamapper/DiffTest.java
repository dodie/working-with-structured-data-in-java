...
/**
 * Compare and show differences of Maps, POJOs and JSON Structures.
 * 
...
public class DiffTest {
	...
	Map<String, Object> original = objectMapper().readValue(new File(jsonPath), Map.class);
	
	final String etalonJsonPath = getClass().getClassLoader().getResource("record_etalon.json").getFile();
	Map<String, Object> etalon = objectMapper().readValue(new File(etalonJsonPath), Map.class);
	...
	final String jsonPath = getClass().getClassLoader().getResource("record.json").getFile();
	Person original = objectMapper().readValue(new File(jsonPath), Person.class);
	
	final String etalonJsonPath = getClass().getClassLoader().getResource("record_etalon.json").getFile();
	Person etalon = objectMapper().readValue(new File(etalonJsonPath), Person.class);
